...
Benchmarks the port scanner under different conditions:
  1. Sequential vs Concurrent scan time
  2. Concurrent scan with increasing thread counts
  3. Varying port range sizes

Outputs results to terminal + saves an HTML performance report.
...
TARGET      = "127.0.0.1"    # scan localhost (always available)
TIMEOUT     = 0.3            # use lower timeout for benchmarking
RETRIES     = 0

# ──────────────────────────────────────────────
# Test 1: Sequential vs Concurrent (fixed range)
# ──────────────────────────────────────────────
def test_sequential_vs_concurrent():
    ...
    print(f"  Speedup      : {speedup}x faster with concurrency")
...
# ──────────────────────────────────────────────
# Test 2: Thread count vs scan time
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# Test 3: Port range size vs scan time
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
def generate_report(test1, test2, test3):
...
<style>
  ...
  * {{ box-sizing: border-box; margin:0; padding:0; }}
  ...
  h1 {{ font-size:2.2rem; font-weight:800; background:linear-gradient(135deg,var(--accent),var(--accent2));
        -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; margin-bottom:.3rem; }}
  .subtitle {{ color:var(--muted); font-size:.9rem; margin-bottom:2.5rem; font-family:'Share Tech Mono',monospace; }}
  .cards {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:1rem; margin-bottom:2.5rem; }}
  .card {{ background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:1.2rem 1.5rem; position:relative; overflow:hidden; }}
  .card::before {{ content:''; position:absolute; top:0; left:0; right:0; height:2px;
                   background:linear-gradient(90deg,var(--accent),var(--accent2)); }}
  .card-label {{ font-size:.7rem; text-transform:uppercase; letter-spacing:.12em; color:var(--muted); margin-bottom:.4rem; }}
  .card-value {{ font-family:'Share Tech Mono',monospace; font-size:1.5rem; color:var(--green); font-weight:700; }}
  .section {{ background:var(--surface); border:1px solid var(--border); border-radius:10px;
              padding:1.5rem; margin-bottom:2rem; }}
  .section h2 {{ font-size:1rem; font-weight:700; margin-bottom:1.2rem; color:var(--accent); text-transform:uppercase; letter-spacing:.1em; }}
  ...
  .bar-label {{ font-size:.75rem; color:var(--muted); margin-bottom:.5rem; font-family:'Share Tech Mono',monospace; }}
  .bar-fill {{ height:36px; border-radius:6px; display:flex; align-items:center; padding:0 1rem;
               font-family:'Share Tech Mono',monospace; font-size:.9rem; font-weight:700; }}
...
<div class="wrap">
  ...
  <div class="subtitle">Custom Port Scanner · Target: {TARGET} · Timeout: {TIMEOUT}s/port</div>
  ...
  <div class="cards">
    ...
    <div class="card">
      ...
      <div class="card-value">{t2_threads[t2_times.index(min(t2_times))]}</div>
    </div>
  </div>

  <!-- Test 1 bar comparison -->
  <div class="section">
    <h2>Test 1 · Sequential vs Concurrent (ports 1–200)</h2>
  ...
  <!-- Test 2 chart -->
  <div class="section">
    <h2>Test 2 · Thread Count vs Scan Time (ports 1–300)</h2>
    <canvas id="chartThreads"></canvas>
  </div>

  <!-- Test 3 chart -->
  <div class="section">
    <h2>Test 3 · Port Range Size vs Scan Time (100 threads)</h2>
  ...
  <div class="section">
    ...
    <p style="line-height:1.7;color:#a0b4cc;">
      ...
      scanning 200 ports takes approximately {test1['sequential_time']}s — one second per closed port.<br/><br/>
      <strong style="color:var(--green)">Concurrent scanning</strong> uses a thread pool to probe multiple
      ports simultaneously. With 100 threads, the same 200-port range completes in {test1['concurrent_time']}s —
      a <strong style="color:var(--green)">{test1['speedup']}x speedup</strong>.<br/><br/>
      Thread count scaling (Test 2) shows diminishing returns beyond ~100 threads, as the OS thread
      scheduling overhead starts to offset gains from parallelism.<br/><br/>
      Port range scaling (Test 3) shows near-linear growth — doubling the range roughly doubles the
      scan time for sequential, while concurrent mode handles larger ranges much more efficiently
      due to parallelism.
    </p>
  </div>

  <footer>Custom Port Scanner · Computer Networks Project · B.Tech CSE 2nd Year</footer>
</div>
...
new Chart(document.getElementById('chartRange'), {{
  type: 'bar',
  data: {{
    labels: {json.dumps([f'1-{{r}}' for r in t3_ranges])},
    datasets: [{{
      label: 'Scan Time (seconds)',
...
if __name__ == "__main__":
    ...
    test1 = test_sequential_vs_concurrent()
    test2 = test_thread_scaling()
    test3 = test_range_scaling()
    generate_report(test1, test2, test3)
